...
#  Modified by Yuan Chai at 1/16/25
#  Previously the word level consistently selected interval 2. Now the interval of word level equal to the interval of vowel level. That means this script allow only one vowel per word.
#######################################################################
...
for j from 1 to number_files
	...
	for k from 1 to number_intervals
        ...
        if word == 0
            ...
            word_label$ = Get label of interval... 'word' 'k'
        endif
	    ...
	    if vowel_label$ <> ""
            ...
            #save the word that the vowel is contained in to a temporary soundfile for anaylsis
						select TextGrid 'soundname$'
						word_start = Get starting point... 'word' 'k'
						word_end = Get end point... 'word' 'k'
						select Sound 'soundname$'
